...
@SpringBootTest // Were testing spring
...
class ArtifactControllerTest {
    ...
    @MockitoBean // Since were testing the controller, we need spring contexts, since were mocking the
    ArtifactService artifactService;

    @Autowired
    ObjectMapper objectMapper; // Best way to serialize objects to JSON, since the frontend sends JSON only

    List<Artifact> artifacts;

    @Value("${api.endpoint.base-url}") // How we inject the base url from the YAML to var
    String baseUrl;
    @BeforeEach
    void setUp() {
    ...
    void testFindArtifactByIdSuccess() throws Exception {
        // Given
        given(this.artifactService.findById("1250808601744904191")).willReturn(this.artifacts.getFirst());

        // When and then
        this.mockMvc.perform(get(baseUrl + "/artifacts/1250808601744904191").accept(MediaType.APPLICATION_JSON)) // Were crating a fake HTTP get request
                .andExpect(jsonPath("$.flag").value(true)) // Compare return response and expected balues
    ...
    void testFindArtifactByIdNotFound() throws Exception {
        ...
        // When and then
        this.mockMvc.perform(get(baseUrl + "/artifacts/1250808601744904191").accept(MediaType.APPLICATION_JSON)) // Were crating a fake HTTP get request
                .andExpect(jsonPath("$.flag").value(false)) // Compare return response and expected balues
    ...
    void testAddArtifactSuccess() throws Exception {
        //Given - prepare a fake DTO record
       ...
       String json = this.objectMapper.writeValueAsString(artifactDto); // Serialize string

        Artifact savedArtifact = new Artifact();
        savedArtifact.setId("1250808601744904197");
        savedArtifact.setId("RemembrAll");
        ...
        given(this.artifactService.save(Mockito.any(Artifact.class))).willReturn(savedArtifact); // Given an artifact, return it



        // When and Then
        this.mockMvc.perform(post(baseUrl +"/artifacts").contentType(MediaType.APPLICATION_JSON).content(json).accept(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.flag").value(true))
                .andExpect(jsonPath("$.code").value(StatusCode.SUCCESS))
    ...
    void testUpdateArtifactSuccess() throws Exception {
        ...
        updatedArtifact.setDescription("A new description."); // Just changed the description
        updatedArtifact.setImageUrl("ImageUrl");

        given(this.artifactService.update(eq("1250808601744904192"),Mockito.any(Artifact.class))).willReturn(updatedArtifact); // Given an artifact, return it



        // When and Then
        this.mockMvc.perform(put(baseUrl + "/artifacts/1250808601744904192").contentType(MediaType.APPLICATION_JSON).content(json).accept(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.flag").value(true))
                .andExpect(jsonPath("$.code").value(StatusCode.SUCCESS))
    ...
    void testUpdateArtifactErrorWithNonExistentId() throws Exception {
        ...
        given(this.artifactService.update(eq("1250808601744904192"),Mockito.any(Artifact.class))).willThrow(new ObjectNotFoundException("artifact","1250808601744904192")); // Given an artifact, return it
